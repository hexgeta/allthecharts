...
interface FertilityChartProps {
  ...
  title?: string
  description?: string
  /** Draw the replacement-rate reference line at y = 2.1 (only meaningful for fertility rate) */
  showReplacementLine?: boolean
  /** Shade the smartphone / mobile-internet rollout window */
  ...
  return (
    <Card className="bg-black/20 border-gray-800 overflow-hidden">
      <CardHeader className="pb-2">
        <CardTitle className="text-white text-lg">{title || indicator.name}</CardTitle>
        {description && (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 16, right: 16, left: 6, bottom: 5 }}>
                ...
                {smartphoneBand && (
                  ...
                  >
                    <Label
                      value="Smartphones + mobile internet go mainstream"
                      position="insideTop"
                      fill="#c4b5fd"
                      fontSize={11}
                      offset={10}
